...
public class CartService {
    ...
    public CartService(CartRepository cartRepository,
                       ...
                       ProductVariantService productVariantService) { // 🟢 Thêm ở đây
    ...
    public CartResponse getCart(Long userId) {
        ...
        for (CartItem it : items) {
            ...
            BigDecimal currentPrice = productVariantService
                    .getVariantDTOsByProductId(variant.getProduct().getId())
                    .stream()
            ...
            r.setImageUrl(productMediaRepository
                    .findFirstByProduct_IdAndIsPrimaryTrueOrderByIdAsc(p.getId())
                    .map(ProductMedia::getUrl)
                    .orElseGet(() -> productMediaRepository
                            .findFirstByProduct_IdOrderByIdAsc(p.getId())
                            .map(ProductMedia::getUrl)
                            .orElse(null)));
            ...
            BigDecimal toppingTotal = ts.stream()
                    .map(CartItemToppingDTO::getPrice)
                    .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal line = currentPrice.add(toppingTotal)
    ...
    public List<CartItemDetail> getItemDetailsByIds(Long userId, List<Long> cartItemIds) {
        ...
        return cartItems.stream().map(ci -> {
            ...
            BigDecimal currentPrice = productVariantService
                    ...
                    .findFirst()
                    .map(ProductVariantDTO::getPrice)
                    .orElse(ci.getPriceAtAddition());

            List<CartItemDetail.ToppingEntry> toppings = ci.getToppings().stream()
                    .map(t -> new CartItemDetail.ToppingEntry(
                            t.getTopping().getId(),
                            t.getTopping().getName(),
                            t.getPriceAtAddition()
                    ))
                    .toList();

            BigDecimal toppingTotal = toppings.stream()
                    .map(CartItemDetail.ToppingEntry::getPrice)
                    .reduce(BigDecimal.ZERO, BigDecimal::add);
            ...
            return new CartItemDetail(
                    ...
                    ci.getQuantity(),
                    ci.getNote(),
                    currentPrice,       // ✅ giá đã có khuyến mãi
                    toppingTotal,
                    toppings
            );
        }).toList();
